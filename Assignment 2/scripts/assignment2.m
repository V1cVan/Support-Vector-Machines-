...
gammas = [0.0001, 0.001, 0.01, 0.1, 1, 10, 100,  10^3, 10^4, 10^5, 10^6]; 
sigma2s = [0.0001,0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000, 100000, 1000000]; 

mse_list = zeros(length(gammas), length(sigma2s));
type = 'function estimation';
kernel = 'RBF_kernel';
for gam_idx = [1:length(gammas)]
    for sig2_idx = [1:length(sigma2s)]
        gam = gammas(gam_idx);
        sig2 = sigma2s(sig2_idx);
        ...
        mse_list(gam_idx,sig2_idx) = RMSE;
...
figure;
surf(-log10(gammas), -log10(sigma2s), mse_list)
colorbar
view(35,35)
xlabel('log10(\gamma)')
...
grid on 
fig = gcf;
exportgraphics(fig, '../figures/1_2/hyper_tuning_results.pdf', 'ContentType', 'vector', 'Resolution', 300);
...
figure() 
sig2e = bay_errorbar ({ X(:,1) , Y , 'f', gam , sig2 }, 'figure');
hold on 
...
grid on 
fig = gcf;
figname = '../figures/2_3/order50.pdf';
exportgraphics(fig, figname, 'ContentType', 'vector', 'Resolution', 300);



order_list = [5 10 20 30 50 70 100 300];
n = length(order_list);
...
[min_rmse, min_idx] = min(RMSE_list);
order = order_list(min_idx)
