...
%             contour(Xax,Yax,reshape(proj_lin(:,1),length(Yax),length(Xax)));
            xdl=dat*U_lin(:,1)*U_lin(:,1)';
            plot(xdl(:,1),xdl(:,2),'r+');
            title('Linear Kernel PCA (first component)');
            xlabel('X_1');
            ylabel('X_2');
            fig = gcf;
            figname = '../figures/1_1/linear_kpca.pdf';
            exportgraphics(fig, figname, 'ContentType', 'vector', 'Resolution', 300);
...
sig2list=[0.0001, 0.001, 0.01, 0.1, 1 , 10, 100, 1000] %[0.001 , 0.01 , 0.1 , 1, 10, 100];
nclist = [1,2,4,8,16,32,64,128];
tr_errors = zeros(length(sig2list),length(nclist));
test1_errors = zeros(length(sig2list),length(nclist));
test2_errors = zeros(length(sig2list),length(nclist));
...
for j = 1:length(sig2list)
    ...
    for i = 1:length(nclist)
        nc = nclist(i)
        % kernel PCA
        [lam,U,scores, omega, recError_tr] = kpca(Xtr,'RBF_kernel',sig2,X,'eigs',nc); 
        [lam,U,scores, omega, recError_test1] = kpca(Xtr,'RBF_kernel',sig2,Xtest1,'eigs',nc); 
        [lam,U,scores, omega, recError_test2] = kpca(Xtr,'RBF_kernel',sig2,Xtest2,'eigs',nc); 
        
        %calculate reeconstruction error
        rec_er_tr = sum(recError_tr, 'all');
        rec_er_test1 = sum(recError_test1, 'all');
        rec_er_test2 = sum(recError_test2, 'all');
        
        %store the errors
        tr_errors(i,j) = rec_er_tr;
        test1_errors(i,j) = rec_er_test1;
        test2_errors(i,j) = rec_er_test2;

    end
end        
figure;    
hm = surf(log(sig2list), nclist, tr_errors);
xlabel('log(sigma2)')
ylabel('Number of components')
zlabel('Reconstruction error')
fig = gcf;
view(130,45)
figname = '../figures/2_1/KPCA_recon_training.pdf';
exportgraphics(fig, figname, 'ContentType', 'vector', 'Resolution', 300);
% hm.Title = 'reconstruction error on the training set';
% hm.XLabel = 'sigmafactor';
% hm.YLabel = 'nc';
% hm.Colormap = jet;

figure;    
hm = surf(log(sig2list), nclist, test1_errors);
xlabel('log(sigma2)')
ylabel('Number of components')
zlabel('Reconstruction error')
fig = gcf;
view(130,45)
figname = '../figures/2_1/KPCA_recon_test1.pdf';
exportgraphics(fig, figname, 'ContentType', 'vector', 'Resolution', 300);

% figure;
% hm = heatmap(sig2list, nclist, test1_errors);
% hm.Title = 'reconstruction error on testset 1';
% hm.XLabel = 'sigmafactor';
% hm.YLabel = 'nc';
% hm.Colormap = jet;

figure;    
hm = surf(log(sig2list), nclist, test2_errors);
xlabel('log(sigma2)')
ylabel('Number of components')
view(130,45)
zlabel('Reconstruction error')
fig = gcf;
figname = '../figures/2_1/KPCA_recon_test2.pdf';
exportgraphics(fig, figname, 'ContentType', 'vector', 'Resolution', 300);


% figure;
% hm = heatmap(sig2list, nclist, test2_errors);
% hm.Title = 'reconstruction error on testest 2';
% hm.XLabel = 'sigmafactor';
% hm.YLabel = 'nc';
% hm.Colormap = jet;
